...
export async function importFileToLocalStorage(filename) {
  ...
  if (filename.endsWith(".json")) {
    ...
    loadJSONDataToLocalStorage({
      ...{ sections: data.sections, metaData: data.metaData }
    });
  }
  return data;
}
...
export async function saveScreenwriterFile(
  fileName = null,
  { metaData, sections } = {},
  { saveHistory, increaseLastNumberOnFilename } = {}
) {
  ...
  if (increaseLastNumberOnFilename === undefined) {
    increaseLastNumberOnFilename = localStorage.getItem('increaseLastNumberOnFilename') === 'true'
  }

  async function exportDataOfCurrentScreenplay(format) {
    let data = { sections, metaData };

    let content = null;
    if (format === "json" || format === "screenwriter") {
      content = JSON.stringify(data);
    } else {
      content = Exporter(data.sections, data.metaData, { excludeMetaData: localStorage.getItem('excludeMetaDataOnPlainText') === 'true' });
    }
    return content;
  }
  ...
  if (!fileName) {
    ...
    localStorage.setItem("lastStoredFile", fileName);
  } else {
    newFilename = fileName;
  }
  ...
  if (saveHistory) {
    ...
    if (screenplay) {
      ...
      let newHistoryEntry = {
        ...
        payload: deltaOfData(
          currentScreenplay.sections.map((s) => {
            return {
              html: s.html,
