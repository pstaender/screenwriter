    ...
    return (
        <div className="edit-meta-data">
            <div className="window">
                {metaData && Object.entries({...DEFAULT_META_DATA, ...metaData}).map((kv) => (
                    <textarea key={`metadata_${kv[0]}`} onChange={(ev) => setMetaDataByKey(kv[0], ev.target.value)} value={kv[1]} placeholder={Voca.capitalize(kv[0])}></textarea>
                ))}
                <div className="icon">
                    <div>
