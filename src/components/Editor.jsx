...
export function Editor({
  ...
  function sectionsCurrentScreenplayViaLocalStorageOrPlainText() {
    let lastScreenPlay = {};
    try {
      lastScreenPlay = localStorage.getItem("currentScreenplay")
        ? JSON.parse(localStorage.getItem("currentScreenplay"))
        : {};
    } catch (e) {
    ...
    if (lastScreenPlay.sections) {
      ...
      if (lastScreenPlay.sections.constructor !== Array) {
        console.error(
          `lastScreenPlay.sections is not an array (there is something broken). We reset the local storage here`
        );
      ...
      return [
        ...lastScreenPlay.sections.map((s, i) => {
          return {
  ...
  function resetDocumentAndStartDemo() {
    ...
    document
      .querySelector("#screenwriter > .focus")
  ...
  function insertNewSectionAtIndex(index, { html, id, classification } = {}) {
    setSections([
      ...setAllSectionToNotCurrent(sections.slice(0, index)),
      ...emptySection({ html, id, classification, chooseEditingLevel: true }),
      ...setAllSectionToNotCurrent(sections.slice(index))
    ]);
  }
  ...
  function insertNewSectionAfterId(
    currentID,
    { html, id, classification } = {}
  ) {
    if (html) {
      // trim
  ...
  function insertNewSectionBeforeId(
    currentID,
    { html, id, classification } = {}
  ) {
    let index = sections.indexOf(sections.filter((s) => s.id === currentID)[0]);
  ...
  function getNext(id) {
    return (
      sections[sections.indexOf(sections.filter((s) => s.id === id)[0]) + 1] ||
      null
    );
  }
  ...
  return (
    <>
      ...
      >
        {sections.map((section, i) => (
          <SceneSection
            ...
            cursorToEnd={section.cursorToEnd}
            setCurrentSectionById={setCurrentSectionById}
            insertNewSectionAfterId={insertNewSectionAfterId}
            insertNewSectionBeforeId={insertNewSectionBeforeId}
            findSectionById={findSectionById}
            randomID={randomID}
            chooseEditingLevel={section.chooseEditingLevel || false}
            sections={sections}
      ...
      {sections && sections.length > 0 && (
        <Toc sections={sections} show={showToc} setShow={setShowToc}></Toc>
      )}
    </>
  );
}
